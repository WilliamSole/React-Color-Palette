...
const styles = {
    ...
    copyText: {
        color: props => chroma( props.background ).luminance() >= 0.065 ? 'black' : 'white'
    },
    colorName: {
        color: props => chroma( props.background ).luminance() <= 0.075 ? 'white' : 'black'
    },
    seeMore: {
        color: props => chroma( props.background ).luminance() >= 0.065 ? 'black' : 'white',
...
class ColourBox extends Component {
    ...
    render() {
        const { classes, name, background, paletteId, id, showingFullPalette } = this.props;
        const { copied } = this.state;
        const isLightColor = chroma( background ).luminance() >= 0.065;
